...
# ════════════════════════════════════════════════════════
    ...
    # ── Reconnect Account ───────────────────────────────────────────────────────
    st.subheader("🔗 Reconnect Account")
    if central_enabled:
            ...
            if _rc_submit and _rc_email.strip() and _rc_password:
                ...
                if _rc_result.get("ok"):
                    st.success(
                        ...
                        "Restart the node for the new secret to take full effect."
                    )
                    st.balloons()
                    st.cache_data.clear()
    ...
    # ── Vision Capability ────────────────────────────────────────────────────
    st.subheader("👁 Vision Capability")
    st.markdown(
        "<div class='cm-card' style='padding:14px 16px'>"
        "<b style='color:#065f46'>✅ Enabled — required</b><br/>"
        "<span style='font-size:13px;color:#374151'>"
        "Vision (image understanding) is a compulsory capability for every ChainMind node "
        "and cannot be turned off. It is always advertised alongside text."
        "</span></div>",
        unsafe_allow_html=True,
    )
    st.toggle(
        ...
        value=True,
        disabled=True,
        help="Vision is required for all nodes and cannot be disabled.",
        key="vision_capability_toggle",
    )
